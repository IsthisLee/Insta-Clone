...
router.post("/userlist/register", async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(400).send({
      errorMessage: "요청한 데이터 형식이 올바르지 않습니다.22",
    });
    return;
  }
});
...
router.post("/userlist/login", async (req, res) => {
  try {
    ...
    const userFind = await Users.findOne({ userId, password }).exec();
    if (!userFind) {
      res.status(400).send({
    ...
    const token = jwt.sign({ userId: userFind.userId }, secretKey); //토큰생성
...
//유저 프로필 페이지
router.get("/userpforile/:userId", async (req, res, next) => {
  try {
